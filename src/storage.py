...
class Storage:
    def __init__(self, db_path=None, out_dir=None):
        ...
        db_path = db_path or os.getenv("SQLITE_DB", os.path.join(out_dir, "iocs.db"))

        os.makedirs(out_dir, exist_ok=True)
        self.db_path = db_path
        self.out_dir = out_dir
        self._conn = sqlite3.connect(self.db_path, check_same_thread=False)
        self._ensure_tables()   # <-- this method must exist below
    ...
    def export_json(self, filename=None):
        filename = filename or os.path.join(
            self.out_dir,
            f"iocs_{datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')}.json"
        )
        cur = self._conn.cursor()
